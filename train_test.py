...
def main(config):
    ...
    if config.mode == 'train':  # train mode
        if config.auto_resume_training_from_lastest_steps: #True
            start_train_step, start_train_epoch = base.resume_last_model()
        # continual loop
        for current_step in range(start_train_step, loaders.total_step): 
            current_total_train_epochs = config.total_continual_train_epochs if current_step > 0 else config.total_train_epochs # 50
            ...
            for current_epoch in range(start_train_epoch, current_total_train_epochs): # epoch: range(0,50)
                visdom_result_dict = {}
                ...
                if current_epoch < config.epoch_start_joint: # --epoch_start_joint : default=80, help='start epoch for start joint sampled'
                ...
                if config.output_featuremaps: #False
                ...
                if config.test_frequency > 0 and current_epoch == config.total_train_epochs - 1:
                    
                    rank_map_dict, rank_map_str = fast_test_p_s(config, base, loaders, current_step, if_test_forget=config.if_test_forget) #--if_test_forget,default=True,help='test during train for forgeting'
                    logger(
...
if __name__ == '__main__':
    ...
    machine_dataset_path = 'E:\\lzs\\GwFReID\\datasets'
    ...
    parser.add_argument('--test_dataset', nargs='+', type=str,default=['market', 'subcuhksysu','duke', 'msmt17','cuhk03','grid', 'sensereid',
                                 'cuhk01','cuhk02','viper','ilids','prid'])
    ...
    parser.add_argument('--flag', type=int, default=0, help='image transformation')
    ...
    # parser.add_argument('--steps', type=int, default=250, help='150 for 5s32p4k, 75 for 10s32p4k')
    ...
    parser.add_argument('--resume_test_model', type=str, default='E:\\lzs\CGReID_new\\results\\2023-03-13-17-09-58\\models\\4\\model_tasknet_49.pkl', help='')   
    ...
    parser.add_argument('--resume_visualize_model', type=str, default='/media/lzs/de2ef254-eaa4-4486-b00b-ab367ed2a6d8/home/lzs/LifelongReID/results/2022-05-22-13-57-36/models/0/model_tasknet_33.pkl',
    ...
    parser.add_argument('--weight_r', type=float, default=2, help='weight for fd loss')
    ...
    parser.add_argument('--weight_kd', type=float, default=1, help='weight for disstilation loss')
